...
$(function() {
  ...
  function sortChar() {
    ...
    $(".initials ul li").click(function() {
      ...
      } else if (_index == $(".initials ul").children('li').length - 1) {
  ...
  function initials() { //公众号排序
    ...
    for (var differChar = 0; differChar < initials.length; differChar ++) {
      charList += '<li>'+initials[differChar]+'</li>';
    }
    charList += '<li>#</li>';
    $('.initials').find('ul').append(charList);
    ...
    var elList = document.getElementsByClassName('sort_list')
    for (var j = 0; j < elList.length; j ++) {
      // console.log(elList[j].nextSibling.getAttribute('class'))
      if (elList[j].nextSibling.getAttribute('class') === 'sort_list') {
        elList[j].style.borderBottom = '0.5px solid #eee'
      }
    }
  }
})
